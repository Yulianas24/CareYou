<nav class="duration-500 top-0 flex justify-between fixed items-center w-screen h-18 bg-white z-50 laptop:justify-evenly"
    id="nav">
                        ...
                        <ul class="py-1 text-sm text-gray-700 dark:text-gray-200">
                            ...
                            <li>
                                <form action="/logout" method="post">
                                    @csrf
                                    <button type="submit"
                                        class="block py-2 px-4 text-black w-full text-left hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Logout</button>
                                </form>
                            </li>
                        </ul>
                    </div>
    ...
    <div class="hidden laptop:flex laptop:items-center ">
        ...
        <a href="#" class="peer px-1 block relative">
            ...
            {{-- !: Name user --}}
            <button loginText class="peer px-1 flex text-black text-md">
            ...
            <div
                ...
                <ul class="py-1 text-sm text-gray-700 dark:text-gray-200">
                    @if (auth()->user()->level === 'konselor')
                        <li>
                            <a href="/dashboard"
                                class="block py-2 px-4 text-white hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Dashboard</a>
                        </li>
                    @endif
                    <li>
                        <a href="/profile/{{ auth()->user()->username }}/edit"
                            class="block py-2 px-4 text-white hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Profil</a>
                    </li>
                    <li>
                        <form action="/logout" method="post">
                            @csrf
                            <button type="submit"
                                class="block py-2 px-4 text-white w-full text-left hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Logout</button>
                        </form>
                    </li>
                </ul>
            </div>
