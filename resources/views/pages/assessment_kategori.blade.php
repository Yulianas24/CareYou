        ...
        <div class="w-full flex space-y-8 flex-col items-start justify-center laptop:w-1/2">
            ...
            <div class="w-full shadow-md rounded-lg h-fit space-y-6 p-8">
                ...
                <div class="flex flex-col">
                    <form class="flex justify-between" method="POST" action="/assessment/kategori">
                        @csrf
    ...
    <script>
        ...
        function initOption() {
            if(asessments[index_page].type == 'options'){
                ...
                document.getElementById('set-option').innerHTML = 
                `<div>
                    <textarea onChange="getEssayAnswer('essay_${asessments[index_page].id}')" value="${answers[index_page] ?? ''}" id="essay_${asessments[index_page].id}" name="${asessments[index_page].id}" class="w-full p-2 border border-blue-200 rounded-lg placeholder="Input data" rows="4">${answers[index_page] ?? ''}</textarea>
